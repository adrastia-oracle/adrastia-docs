...
Credit Extension Oracles (CEO) are purpose-built for lending protocols to extend credit — **slowly**. By aggregating a liquidation oracle and a median filtered daily average oracle, CEOs can be used to extend credit to a borrower in a way that is safe for the protocol. Price movements have to be sustained for a period of time (typically days) before the CEO will extend credit at the new price.
