...
The time-weighted value accumulation is used to calculate time-weighted averages of the data point value over a period of time. The logic for calculating the time-weighted value is determined by the hard-set [averaging strategy](/contracts/strategies/averaging). [TWA[P] oracles](/contracts/oracles/twap) provide the logic for calculating and storing these averages, utilizing the accumulator contract.
