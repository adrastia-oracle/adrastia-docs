---
title: DEX Feed Attack Vectors
description: Learn how Adrastia mitigates DEX oracle attacks.
---

import { Callout } from 'nextra/components';

# DEX Feed Attack Vectors
...
<Callout emoji="🧿">
  Only EOAs can perform accumulator updates. Smart contracts are not able to update. Furthermore,
  flash loans cannot be used.
</Callout>
...
<Callout emoji="🧿">
  Accumulator update transactions validate that the data being recorded holds for at least one
  block.
</Callout>
...
<Callout emoji="🧿">
  Pools with insufficient liquidity levels will be excluded from aggregation to prevent situations
  where unprofitable arbitrage opportunities prevent accurate pricing.
</Callout>
...
<Callout emoji="🧿">
  Specify a maximum age of the consultation's observation to prevent outdated data from being
  consumed. Consider using a fallback oracle when data is stale.
</Callout>

At the updater level,

<Callout emoji="🧿">
  Submit update transactions directly to miners while avoiding the public mempool to avoid being
  front-run.
</Callout>
